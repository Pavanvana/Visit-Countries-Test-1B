...
class VisitCountries extends Component {
  state = {
    visitedCountrisList: initialCountriesList,
  }

  removeCountry = id => {
    const {visitedCountrisList} = this.state
    const removedCountry = visitedCountrisList.map(each => {
      if (each.id === id) {
        const updatedItem = {
          ...each,
    ...
    this.setState({visitedCountrisList: removedCountry})
  }

  renderVisitedCountries = () => {
    const {visitedCountrisList} = this.state
    const upDatedList = visitedCountrisList.filter(
      eachItem => eachItem.isVisited === true,
    )
    return upDatedList.length === 0 ? (
      <div className="no-visited-place">
  ...
  visitCountry = id => {
    const {visitedCountrisList} = this.state
    const visitedCountry = visitedCountrisList.map(each => {
      if (each.id === id) {
        const updatedountry = {
          ...each,
          isVisited: !each.isVisited,
        }
        return updatedountry
      }
      return each
    })
    this.setState({visitedCountrisList: visitedCountry})
  }
    ...
    return (
      <div className="main-container">
        ...
        <ul className="countries-container">
          {visitedCountrisList.map(eachCountry => (
            <EachCountry
              key={eachCountry.id}
